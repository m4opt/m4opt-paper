...
\begin{deluxetable*}{lrcccccccccccrc}
    ...
    \tablenotetext{e}{\ac{GW} sky map probability density (deg$^{-2}$).}
    \tablenotetext{f}{Mean \ac{GW} distance estimate (Mpc) over the \ac{UVEX} field.}
    \tablenotetext{g}{Median limiting magnitude (AB mag) over the \ac{UVEX} field.}
    \tablenotetext{h}{Median dust extinction (AB mag) in band over the \ac{UVEX} field.}
    \tablenotetext{i}{Median sky brightness (AB mag arcsec$^-2$) over the \ac{UVEX} field.}
    \tablenotetext{j}{Contribution to the probability of detection given this field.}
