...
\affiliation{Artemis, Observatoire de la C\^{o}te d'Azur, Universit\'{e} Côte d'Azur, Boulevard de l'Observatoire, F-06304 Nice, France}
\affiliation{School of Physics and Astronomy, University of Minnesota, Minneapolis, MN 55455, USA}
...
\author[0009-0008-9546-2035]{Andrew Toivonen}
\affiliation{Minnesota Institute for Astrophysics, University of Minnesota, Minneapolis, MN 55455, USA}
\affiliation{School of Physics and Astronomy, University of Minnesota, Minneapolis, MN 55455, USA}

\author[0000-0001-5857-5622]{Hannah Earnshaw}
...
The \texttt{dorado-scheduling} package took into consideration the time-varying \ac{FoR} of the telescope and employs the formalism of \ac{MILP} to find the globally optimal sequence of observations that covers the largest integrated probability within the \ac{GW} localization. Responding to inquiries during the Dorado \ac{SV} about varying extinction and foregrounds, we added to the scheduler a powerful capability to dynamically optimize the exposure time of each field to adjust for spatial variations in sensitivity. In our simulations, this code greatly improved the detection efficiency of the mission. However, this code remained on a Git branch, so it was not used in the \ac{UVEX} study.
...
In this paper, we describe the mathematical approach of \ac{M4OPT} then use it to produce realistic \ac{ToO} observing sequences with \ac{UVEX} for simulated \ac{GW} events. An example \ac{M4OPT} observing plan is shown in Fig~\ref{fig:example}. We show that the new dynamic exposure time capabilities dramatically increase the number of detected \acp{KN} compared to our earlier work in \citet{criswell}.
...
TODO: discuss MILP astronomical scheduling papers: \citep{2014SPIE.9149E..0ES,2019PASP..131f8003B,2022ApJ...935...87P,2024AJ....167...33H}
...
The most basic version of our \ac{ToO} problem is that we want to maximize the total integrated probability that the true, but unknown, position of the source is within one or more of the footprints that we select to observe. The weights $(w_j)_j$ are the values of the probability sky map sampled on a HEALPix grid and the subsets $(S_i)_i$ are the HEALPix pixels contained within the footprints of the \ac{FOV} on a reference grid of allowed pointings (see Fig.~\ref{fig:overlapping-fields}).
...
\begin{deluxetable}{lc}
    ...
    \enddata
    \tablenotetext{a}{We assume an optimal \ac{PSF} photometry with a Nyquist-sampled \ac{PSF}, i.e., a sharpness of $1/4\pi$ \citep{2005MNRAS.361..861M}.}
    \tablenotetext{b}{The function $\phi(x)$ is the standard normal distribution function.}
\end{deluxetable}
...
There are three spatially-dependent effects in the \ac{ETC}:
...
The expected numbers of events selected for follow-up and detected are shown in Table~\ref{tab:selected-detected}.
