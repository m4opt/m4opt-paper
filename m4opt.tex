...
\begin{figure*}
    ...
    \end{interactive}
    \caption{\label{fig:example}An example \ac{M4OPT} observing plan. \textbf{Top panel} and \textbf{inset}: The footprints of the observations filled in pink, the 90\% credible region of the \ac{GW} localization outlined in green, the region that is always outside of the \ac{FOR} (referred to in the legend as the ``averaged'' \ac{FOR}) filled in deep blue, and the region that is outside of the instantaneous \ac{FOR} filled in light blue. \textbf{Bottom panel}: The timeline of the accumulation of detection probability and area. The region that has been covered by one visit is shown in pale red and green, whereas the region that has been covered by two visits is shown in deep red and green. This figure is available as an animation.}
\end{figure*}
...
\begin{deluxetable*}{lrcccccccccccrc}
    ...
    \tablehead{
        &&& \multicolumn3c{Location\tablenotemark{c}}
        & \multicolumn3c{Target\tablenotemark{d}}
        & \multicolumn{2}{c}{Sky map}
        \\
        \colhead{Time\tablenotemark{a}} &
        \colhead{Dur.\tablenotemark{b}} &
        \colhead{Action} &
        \colhead{$x$} &
        \colhead{$y$} &
        \colhead{$z$} &
        ...
        \colhead{Dec.} &
        \colhead{Roll} &
        \colhead{Prob.\tablenotemark{e}} &
        \colhead{Dist.\tablenotemark{f}} &
        \colhead{Lim.\tablenotemark{g}} &
        \colhead{Dust\tablenotemark{h}} &
        \colhead{Sky\tablenotemark{i}} &
        \colhead{Det. Prob.\tablenotemark{j}}
    }
    \startdata
    \input{tables/example.tex}
    \enddata
    \tablenotetext{a}{UTC start time of scheduled action on the arbitrary date of 2012-06-01.}
    \tablenotetext{b}{Duration (s) of scheduled action.}
    \tablenotetext{c}{Cartesian coordinates (Mm) of the spacecraft in the Earth-fixed (\acl{ITRS}) frame.}
    \tablenotetext{d}{Target location (deg) in the equatorial (\acl{ICRS}) frame.}
    ...
    \tablenotetext{f}{\textit{A posteriori} mean \ac{GW} distance estimate (Mpc) over this field.}
...
The reference \ac{ETC} for \ac{UVEX} is still closed source because the instrument is still evolving in small details. We use \ac{M4OPT}'s open-source toy model of the instrument performance that roughly reproduces the public sensitivity curve plot on the \ac{UVEX} web site\footnote{\url{https://www.uvex.caltech.edu/page/for-astronomers}; accessed 2025-01-26} (see Fig.~\ref{fig:etc}). The parameters of the toy model are listed in Table~\ref{tab:etc}.
...
\begin{deluxetable*}{ccrrrrrrcc}
    ...
    \tablehead{
        ...
        \colhead{Run} & \colhead{Event ID} & \colhead{$m_1 / M_\odot$} & \colhead{$m_2 / M_\odot$} & \colhead{$\alpha$/deg} & \colhead{$\delta$/deg} & \colhead{$d_\mathrm{L}$/Mpc} & \colhead{$A_{90\%}$ / deg$^2$} & \colhead{Objective value\tablenotemark{a}} & \colhead{Detection prob.\tablenotemark{b}}
    }
    \startdata
...
This is LIGO document P2500008-v2.
\end{acknowledgments}
